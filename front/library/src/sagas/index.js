...
function getErrorMessage(error) {
  ...
  //TODO replace messages by codes
  if (errorMessage === "invalidate token: Token is expired") {
    return [errorMessage, { type: "LOG_OUT" }];
  } else if (
    errorMessage === "Invalid verification code or user doesn't exists"
  ) {
    return [errorMessage, { type: "LOG_OUT" }];
  }
  return [errorMessage];
...
export function* callServerLastest() {
  yield takeLatest(
    "VERIFY",
    submitGet,
    (action) => "/api/auth/verifyemail/" + action.verifyHash,
    (response) => ({
      verified: true,
    }),
    "SET_VERIFIED"
  ...
  yield takeLatest(
    "DELETE_KNOWTYPE",
    deleteResource,
    (action) => "/api/youknow/knowtype/" + action.knowtype.id,
    (action) => [{ type: "GET_KNOWTYPES" }]
  );
  yield takeLatest(
    "CREATE_KNOWTYPE",
    postResource,
    "/api/youknow/knowtype",
    (request) => request.knowtype,
    (action, result) => [
      { type: "GET_KNOWTYPES", payload: { current: result.data } },
    ]
  );
  yield takeLatest(
    "EDIT_KNOWTYPE",
  ...
  yield takeLatest(
    "GET_KNOWTYPES",
    fetchResource,
    (action) => "/api/youknow/knowtypes",
    (response) => {},
    (action, result) => {
      if (action.payload?.current) {
        return [
          { type: "SET_STATE_KNOWTYPES", payload: { knowtypes: result.data } },
          {
            type: "SET_CURRENT_STATE_KNOWTYPE",
            payload: { knowtype: action.payload?.current },
          },
        ];
      } else {
        return [
  ...
  yield takeLatest(
    "DELETE_KNOW",
    deleteResource,
    (action) => "/api/youknow/know/" + action.know.id,
    (action, result) => [
      { type: "DELETE_STATE_KNOW", payload: { know: result.data } },
    ]
  );
  yield takeLatest(
    "CREATE_KNOW",
    postResource,
    "/api/youknow/know",
    (request) => request.know,
    (action, result) => [
      { type: "CREATE_STATE_KNOW", payload: { know: result.data } },
    ]
  );
  yield takeLatest(
    "EDIT_KNOW",
    ...
    (request) => request.know,
    (action, result) => [
      { type: "UPDATE_STATE_KNOW", payload: { know: result.data } },
    ]
  );
  yield takeLatest(
    "GET_KNOWS",
    fetchResource,
    (action) => "/api/youknow/knows/" + action.knowtype_id,
    (response) => {},
    (action, result) => [
      { type: "SET_STATE_KNOWS", payload: { knows: result.data } },
    ]
  );
  yield takeLatest(
    "UPLOAD_KNOWS",
    uploadResource,
    (action) => "/api/youknow/knowtypes/" + action.knowtype.id,
    (request) => request.payload.file,
    (action, result) => [
...
function* navigate(path, action) {
  yield call(Navigation.navigate, path);
}
